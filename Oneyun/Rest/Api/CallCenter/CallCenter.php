...
class CallCenter extends Version
{
    ...
    public function deleteExt($id)
    {
        ...
        $response = $this->request('DELETE', $this->getBaseUrl() . self::CALLCENTER_EXTENSION, $params, array());
        return array(
            'statusCode' => $response->getStatusCode(),
            'headers' => $response->getHeaders(),
            'content' => $response->getContent()
     ...
     * @throws OptionsException
     */
    public function agentLogin($name = null, $channel = null, $extension, $options)
    {
        if (empty($name)) {
    ...
    public function agentKeep($agent_name)
    {
        ...
        $response = $this->request('DELETE', $this->getBaseUrl() . self::CALLCENTER_AGENT . "/" . $agent_name, array(), array());
        return array(
            'statusCode' => $response->getStatusCode(),
            'headers' => $response->getHeaders(),
            'content' => $response->getContent()
     ...
     * @return array
     */
    public function setAgentExt($agent_name, $ext_id)
    {
        if (empty($agent_name)) {
            throw new OptionsException('坐席名称必填');
        }
     ...
     * @throws OptionsException
     */
    public function createCondition($channe,$where,$options)
    {
        if (empty($channe)) {
            throw new OptionsException('通道Id必填');
        }
